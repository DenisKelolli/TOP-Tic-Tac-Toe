...
const playGame = function() {
  ...
  gridItems.forEach((item) => {
    item.addEventListener('click', function(event) {
      ...
      //Win conditions for computer (Red)
      } else if (
      ...
      //Tie if either player has not win in 9 clicks
      else if(count === 9){
...
//Clears board once either the player or computer wins or there is a tie.
if (winner.textContent === "Player wins!" || winner.textContent === "Computer wins!" || winner.textContent === "Its a tie!") {
  gridItems.forEach((item) => {
